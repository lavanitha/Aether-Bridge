...
class ApiService {
  ...
  private getMockDashboardData() {
    return {
      academicPassport: {
        ...
        completedCourses: 40,
        gpa: 3.8,
        institutions: ['Harvard University', 'Stanford University', 'MIT'],
        skills: ['Machine Learning', 'Data Science', 'Blockchain Development', 'Software Engineering', 'Artificial Intelligence'],
      },
      verifiedCredits: 95,
      pathwaySuggestions: [
        { id: 'p1', title: 'AI/ML Specialization', description: 'Advanced pathway in AI and machine learning', confidence: 85, courses: ['Advanced ML', 'Deep Learning', 'NLP'] },
        { id: 'p2', title: 'Blockchain Development', description: 'Blockchain and smart contract development', confidence: 78, courses: ['Smart Contracts', 'DeFi', 'Web3'] },
        { id: 'p3', title: 'Data Science', description: 'Data science and analytics pathway', confidence: 92, courses: ['Data Analysis', 'Statistics', 'Big Data'] },
      ],
      recentActivity: [
        { id: 'a1', type: 'course_completed', title: 'Machine Learning Fundamentals', description: 'Completed with grade A', timestamp: new Date(Date.now() - 86400000).toISOString(), status: 'completed' },
        { id: 'a2', type: 'credential_issued', title: 'Blockchain Developer Certificate', description: 'Credential verified on blockchain', timestamp: new Date(Date.now() - 172800000).toISOString(), status: 'completed' },
        { id: 'a3', type: 'application_submitted', title: 'MIT Transfer Application', description: 'Submitted for Computer Science', timestamp: new Date(Date.now() - 259200000).toISOString(), status: 'pending' },
      ],
    };
  }
  ...
  async getCourses(filters?: any): Promise<Course[]> {
    ...
    try {
      const res = await this.makeRequest(`/courses${queryParams}`);
  ...
  private getMockCourses(): Course[] {
    return [
      { id: '1', title: 'Introduction to Computer Science', institution: 'Harvard University', category: 'Computer Science', credits: 3, description: 'Fundamental concepts of computer science and programming', equivalencyMetadata: {}, isFeatured: true, duration: '12 weeks', level: 'beginner' },
      { id: '2', title: 'Machine Learning Fundamentals', institution: 'Stanford University', category: 'Artificial Intelligence', credits: 4, description: 'Introduction to machine learning algorithms and applications', equivalencyMetadata: {}, isFeatured: true, duration: '16 weeks', level: 'intermediate' },
      { id: '3', title: 'Blockchain Technology', institution: 'MIT', category: 'Blockchain', credits: 3, description: 'Understanding blockchain technology and its applications', equivalencyMetadata: {}, isFeatured: true, duration: '10 weeks', level: 'intermediate' },
      { id: '4', title: 'Data Structures and Algorithms', institution: 'UC Berkeley', category: 'Computer Science', credits: 4, description: 'Core data structures and algorithm design', equivalencyMetadata: {}, isFeatured: false, duration: '14 weeks', level: 'intermediate' },
      { id: '5', title: 'Web Development', institution: 'Stanford University', category: 'Software Engineering', credits: 3, description: 'Full-stack web development with modern frameworks', equivalencyMetadata: {}, isFeatured: false, duration: '10 weeks', level: 'beginner' },
    ];
  }

  async getCourseById(id: string): Promise<Course> {
    const res = await this.makeRequest(`/courses/${id}`);
    return res?.data ?? res;
  }
  ...
  async getEnrolledCourses(): Promise<Course[]> {
    try {
      const res = await this.makeRequest('/courses/enrolled');
      const data = res?.data ?? res;
      return Array.isArray(data) ? data : [];
    } catch (err) {
      return [];
