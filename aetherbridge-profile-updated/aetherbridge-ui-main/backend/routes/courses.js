...
// In-memory enrollments: userId -> Set of courseIds (for end-to-end Courses flow)
const enrollmentsByUser = new Map();
function getEnrolledCourseIds(userId) {
  if (!enrollmentsByUser.has(userId)) enrollmentsByUser.set(userId, new Set());
  return enrollmentsByUser.get(userId);
}
...
// GET /api/courses/enrolled – Must be before /:id so "enrolled" is not treated as id
router.get('/enrolled', authenticateToken, async (req, res) => {
  try {
    const userId = req.user?.uid || req.user?.id || 'dev-user-123';
    const db = getDb();
    const courseIds = getEnrolledCourseIds(userId);
    const enrolled = [];
    for (const id of courseIds) {
      const course = await db.findOne('courses', { id });
      if (course) enrolled.push({ ...course, isEnrolled: true });
    }
    res.json({ success: true, data: enrolled });
  } catch (error) {
    console.error('Error fetching enrolled courses:', error);
    res.status(500).json({ success: false, error: 'Failed to get enrolled courses' });
  }
});

// GET /api/courses/categories – Must be before /:id
...
// GET /api/courses/institutions – Must be before /:id
router.get('/institutions', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to get institutions' });
  }
});
...
router.post('/:id/enroll', authenticateToken, async (req, res) => {
  try {
    ...
    const db = getDb();
    const course = await db.findOne('courses', { id: courseId });
    if (!course) {
      return res.status(404).json({ success: false, error: 'Course not found' });
    }
    getEnrolledCourseIds(userId).add(courseId);
    res.json({ success: true, message: 'Successfully enrolled in course', data: { courseId } });
  } catch (error) {
